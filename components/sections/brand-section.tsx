  ...
  return (
    <section id="brand" className="grid gap-7 xl:grid-cols-[.8fr_1.2fr]">
      <div className="rounded-3xl border border-[#ece9f1] bg-white p-5 shadow-[0_18px_50px_rgba(37,27,57,.05)] sm:rounded-4xl sm:p-9">
        ...
        <p className="mt-4 text-sm leading-6 text-[#7b7580]">{t("Blue Gem leads the interface while Brick Red remains a focused accent for moments that need emphasis.")}</p>
        <div className="mt-8 space-y-3">
          {THEME_COLORS.map((color) => (
            <div key={color.hex} className="flex items-center gap-4 rounded-2xl border border-[#ece9f1] p-3"><span className="size-12 shrink-0 rounded-xl" style={{ background: color.value }} /><div className="min-w-0"><p className="text-sm font-bold">{t(color.name)}</p><p className="text-xs text-[#918a96]">{color.hex}</p></div></div>
          ))}
        </div>
      </div>
