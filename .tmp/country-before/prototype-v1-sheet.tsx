...
interface PrototypeV1SheetProps {
  ...
  readonly activeWallet: "php" | "hkd";
  readonly onWorkCountryChange: (country: string) => void;
}

const WORK_COUNTRIES = [
  { name: "Hong Kong", flag: "flag-hong-kong.svg", currency: "HKD" },
  { name: "Singapore", flag: "flag-singapore.svg", currency: "SGD" },
  { name: "Saudi Arabia", flag: "flag-saudi-arabia.svg", currency: "SAR" },
] as const;
...
function SheetContent({ panel, onClose, workCountry, activeWallet, onWorkCountryChange }: PrototypeV1SheetProps & { readonly panel: V1Panel }) {
  ...
  const [currency, setCurrency] = useState(activeWallet === "hkd" ? "PHP" : "HKD");
  const isHongKong = activeWallet === "hkd";
  const walletCurrency = isHongKong ? "HKD" : "PHP";
  const walletName = isHongKong ? "Hong Kong Dollar" : "Philippine Peso";
        ...
        >
          ...
          <fieldset className={styles.workCountries}>
            ...
            {WORK_COUNTRIES.map((country) => {
              ...
              return (
                <label className={`${styles.workCountry}${selected ? ` ${styles.workCountrySelected}` : ""}`} key={country.name}>
                  <input
                    type="radio"
                    name={countryName}
                    value={country.name}
                    checked={selected}
                    onChange={() => selectCountry(country.name)}
                    onClick={() => { if (selected) selectCountry(country.name); }}
                  />
                  <Image src={`/assets/prototype-figma/${country.flag}`} width={32} height={32} alt="" />
                  <span>{selected && country.name === "Hong Kong" ? "Hongkong" : country.name}</span>
                  <Image
        ...
        <div className={styles.content}>
          {panel === "wallets" && <>
            <div className={styles.wallet}>
              <span className={styles.walletCountry}><Image src={`/assets/prototype-figma/${isHongKong ? "flag-hong-kong.svg" : "flag-ph.svg"}`} width={25} height={25} alt="" /> {walletName} <span>{walletCurrency}</span></span>
              <small>Available balance</small>
              <strong>{isHongKong ? "HK$" : "₱"}3,000.00</strong>
            </div>
            <dl className={styles.details}>
              <div><dt>Receiving bank</dt><dd>{isHongKong ? "DBS" : "AUB"}</dd></div>
              <div><dt>Receiving account</dt><dd>735-****-1234</dd></div>
          ...
          {(panel === "transfer" || panel === "exchange") && <>
            ...
            {panel === "transfer" ? <label className={styles.field}>Send to<select value={destination} onChange={(event) => setDestination(event.target.value)}><option>Bank account</option><option>Payso wallet</option></select></label> : <label className={styles.field}>Exchange to<select value={currency} onChange={(event) => setCurrency(event.target.value)}>{isHongKong && <option value="PHP">PHP · Philippines</option>}{WORK_COUNTRIES.filter((country) => country.currency !== walletCurrency).map((country) => <option key={country.currency} value={country.currency}>{country.currency} · {country.name}</option>)}</select></label>}
