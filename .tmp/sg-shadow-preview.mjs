...
const directory = new URL('./sg-shadow-qa/', import.meta.url);
...
try {
  await cdp('Emulation.setDeviceMetricsOverride', {width:390,height:1100,deviceScaleFactor:1,mobile:false});
  ...
  await until(`document.querySelector('[role="dialog"]')`);
  await delay(350);
  await screenshot('singapore-flag-shadow.png');
  console.log(await evaluate(`getComputedStyle(document.querySelector('[class*="singaporeFlag"]')).boxShadow`));
} finally { ws.close(); }