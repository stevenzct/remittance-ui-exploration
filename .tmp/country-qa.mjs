...
const directory = new URL('./country-qa/', import.meta.url);
...
try {
  for (const width of [1440,390,320]) {
    ...
    await delay(400);
    let previous;
    for (const [country,currency,symbol] of [['Singapore','SGD','S$'],['Saudi Arabia','SAR','$'],['Hong Kong','HKD','HK$']]) {
      await evaluate(`document.querySelector('[aria-label="Wallet and offers"]').scrollTop=400`);
      await choose(country,currency,previous,width===1440 && currency!=='HKD');
      const walletText = await evaluate(`document.querySelector('[aria-label="${currency} wallet"]').innerText`);
      assert(walletText.includes(`${symbol}3,000.00`));
      assert(walletText.includes(`${country} · Receiving Account`));
      if (currency !== 'HKD') {
        assert(walletText.includes('Open your receiving account'));
        assert(!walletText.includes('735-'));
      } else assert(walletText.includes('DBS Black Card'));
      const missingImages=await evaluate(`[...document.images].filter(i=>!i.complete||!i.naturalWidth).map(i=>i.src)`);
      assert.deepEqual(missingImages,[]);
      await screenshot(`${currency.toLowerCase()}-${width}.png`);
      await click('[aria-label="Hide balance"]');
      assert(!await evaluate(`document.querySelector('[aria-label="${currency} wallet"]').innerText.includes('3,000.00')`));
      await click('[aria-label="Show balance"]');
      if (currency !== 'HKD') {
        await click('[class*="openReceivingAccount"]');
        await until(`document.querySelector('[role="status"]').textContent.includes('${currency} receiving account setup coming soon')`);
      }
      await click('[class*="allWallets"]');
      await until(`document.querySelector('[role="dialog"]')`);
      const sheetText=await evaluate(`document.querySelector('[role="dialog"]').innerText`);
      assert(sheetText.includes(currency));
      assert(sheetText.includes(currency==='HKD' ? 'DBS' : 'Not opened'));
      await key('Escape');
      await until(`!document.querySelector('[role="dialog"]')`);
      await click('[class*="actions"] > button:first-child');
      await until(`document.querySelector('[role="dialog"]')`);
      assert((await evaluate(`document.querySelector('[role="dialog"]').innerText`)).includes(`Amount in ${currency}`));
      await key('Escape');
      await until(`!document.querySelector('[role="dialog"]')`);
      await click('[class*="actions"] > button:last-child');
      await until(`document.querySelector('[role="dialog"]')`);
      const exchange=await evaluate(`({text:document.querySelector('[role="dialog"]').innerText,options:[...document.querySelectorAll('select option')].map(e=>e.value),value:document.querySelector('select').value})`);
      assert(exchange.text.includes(`${currency} → PHP`));
      assert.equal(exchange.value,'PHP');
      assert(!exchange.options.includes(currency));
      assert.equal(exchange.options.length,3);
      await key('Escape');
      await until(`!document.querySelector('[role="dialog"]')`);
      await click('[aria-label="Philippines home wallet"]');
      assert(await evaluate(`document.querySelector('[aria-label="PHP wallet"]') !== null`));
      assert.equal(await evaluate(`document.querySelector('[aria-label="Philippines home wallet"]').getAttribute('aria-pressed')`),'true');
      await choose(country,currency,country);
      all.push({width,country,currency,walletText,exchange,reselected:true,returnedToPHP:true,missingImages});
      previous=country;
    }
    console.log(`PASS ${width}px: Singapore, Saudi Arabia, Hong Kong, panels, balance visibility, PHP return, reselection and focus`);
  }
  await cdp('Emulation.setEmulatedMedia',{features:[{name:'prefers-reduced-motion',value:'reduce'}]});
  let previous='Hong Kong';
  for (const [country,currency] of [['Singapore','SGD'],['Saudi Arabia','SAR']]) {
    await choose(country,currency,previous);
    previous=country;
  }
  console.log('PASS reduced motion: Singapore and Saudi Arabia');
  assert.deepEqual(errors,[]);
  assert.deepEqual(failedResponses,[]);
  await fs.writeFile(new URL('report.json',directory),JSON.stringify({results:all,reducedMotion:true,errors,failedResponses},null,2));
  console.log('PASS no browser errors or failed requests');
} finally {
  await fs.writeFile(new URL('diagnostics.json',directory),JSON.stringify({errors,failedResponses},null,2));
  ws.close();
}
